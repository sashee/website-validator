...
import { Assertion, LinkLocation, toCanonical, FileFetchResult, isInternalLink, ValidationResultType, getRedirect, FoundPageFetchResult } from "./index.js";
	...
	if (isInternalLink(baseUrl)(link.url)) {
		if (target.data === null) {
			...
			return (await Promise.all([
				...
				link.asserts.flatMap((assert): ValidationResultType[] => {
				...
				(async () => {
					if (link.location.type === "html" && link.location.element.tagName === "a" && toCanonical(baseUrl, indexName)(link.location.url) === toCanonical(baseUrl, indexName)(link.url) && !link.url.includes("#")) {
						return [{
